...
// Ajax PHP script for loading buildings for waypoint editing
<?php
...
var navBarSelectD = document.createElement("select");
	...
	navBarSelectD.addEventListener("change", FRAIS.loadLevelsForWaypoints, false);
...
<?php }
		echo"var navBarSelectDOption".(mysql_num_rows($ergebnis)+1)."=document.createElement(\"option\");
   			navBarSelectDOption".(mysql_num_rows($ergebnis)+1).".value=999;
	    	var navBarSelectDOption".(mysql_num_rows($ergebnis)+1)."Text=document.createTextNode(\"Alle Haeuser\");
	    	navBarSelectDOption".(mysql_num_rows($ergebnis)+1).".appendChild(navBarSelectDOption".($i+1)."Text);
	
	    	navBarSelectD.appendChild(navBarSelectDOption".(mysql_num_rows($ergebnis)+1).");"
?>
